...
class ConvNet(MLPDropout):
    ...
    def __init__(self, U, height, width, filter_hs, conv_non_linear,
        ...
        layer0_input = Words[T.cast(self.x.flatten(), dtype="int32")].reshape(
            (self.x.shape[0], 1, self.x.shape[1], Words.shape[1]))
        self.conv_layers = []
    ...
    def train(self, train_set, test_set,
        ...
        val_model = theano.function([self.index], self.errors(self.y),
             givens={
                 self.x: val_set_x[self.index * batch_size:(self.index + 1) * batch_size],
                 self.y: val_set_y[self.index * batch_size:(self.index + 1) * batch_size]},
                                    allow_input_downcast=True)
        ...
        test_layer0_input = Words[T.cast(x.flatten(),dtype="int32")].reshape(
            (test_size, 1, height, Words.shape[1]))
        for conv_layer in self.conv_layers:
            test_layer0_output = conv_layer.predict(test_layer0_input, test_size)
            test_pred_layers.append(test_layer0_output.flatten(2))
        test_layer1_input = T.concatenate(test_pred_layers, 1)
        test_y_pred = self.predict(test_layer1_input)
        test_error = T.mean(T.neq(test_y_pred, y))
        test_model = theano.function([x,y], test_error, allow_input_downcast=True)
...
if __name__=="__main__":
    ...
    # save model
    mfile = open(model, "w")
    pickle.dump(cnn.params, mfile)
    pickle.dump([p.get_value() for p in cnn.params], mfile)
